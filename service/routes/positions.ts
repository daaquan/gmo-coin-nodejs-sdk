...
export function registerPositionRoutes(app: FastifyInstance) {
  app.get<{ Querystring: (TenantQuery & { symbol?: string; prevId?: string; count?: string; pageSize?: string }) }>(
    '/v1/positions/open',
  ...
  app.get<{ Querystring: (TenantQuery & { symbol?: string }) }>(
    ...
    async (req, reply) => {
      try {
        ...
        const clientType = determineClientType(q.symbol);
        if (clientType !== 'fx') {
          return reply.status(400).send({ error: 'unsupported', detail: 'Position summary is only available for FX symbols' });
        }
